...
int main()
{
	...
	// Closing connections and Winsock. // WE NEVER GET HERE
...
{
	unsigned long flag = 1;
	if(ioctlsocket(id, FIONBIO, &flag) != 0) // Set the socket to be in non-blocking mode.
	{
		cout << "Server: Error at ioctlsocket(): " << WSAGetLastError() << endl;
	}

	//bool wasSocketAdded = false;
...
string handleLang(string lang, string filename)
{

	//size_t endOfFile = filename.find_last_of('.'); 
	//string ending = filename.substr(0, endOfFile); 
	...
	if(strcmp(lang.c_str(), "en") == 0)
	{
		...
		newfileName = newfileName + "-en";
	}

	else if(strcmp(lang.c_str(), "he") == 0)
	{
		newfileName = newfileName + "-he";

	}
	else if(strcmp(lang.c_str(), "fr") == 0)
	{
		newfileName = newfileName + "-fr";
	}
	else
	{
		return filename;
...
string getOrHeadMethod(SocketState* socket)
{
	...
	size_t queryStringStart = uri.find('?');
	if(queryStringStart != string::npos)
	{
		string queryString = uri.substr(queryStringStart + 1);
		size_t langStart = queryString.find("lang=");
		size_t langEnd = queryString.length();

		lang = queryString.substr(langStart + 5, langEnd - (langStart + 5));
		fileName = uri.substr(0, uri.find('?'));
		newFileName = handleLang(lang, fileName);
	}
	...
	if(inFile.fail())
	{
		cout << "Web Server failed to open the file " << filePath << "Error code: " << WSAGetLastError() << endl;
...
{
	string response;
	string uri = path + uriExtractor(socket);
...
int putOrPostFile(SocketState* socket, string& filePath)
{
	int retCode = 200; // OK
	ofstream outPutFile;
	...
	{
		outPutFile.open(filePath); // Truncation
	}
	else
	{ // == POST
		outPutFile.open(filePath, std::fstream::in | std::fstream::out | std::fstream::app); // Append
	}
